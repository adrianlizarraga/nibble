...
bool gen_gasm(Allocator* gen_mem, Allocator* tmp_mem, Scope* scope, const char* output_file)
{
    ...
    List* data_head = &generator.data_lines->buckets;

    for (List* it = data_head->next; it != data_head; it = it->next)
    {
    ...
    List* text_head = &generator.text_lines->buckets;

    for (List* it = text_head->next; it != text_head; it = it->next)
    {
        Bucket* bucket = list_entry(it, Bucket, lnode);
