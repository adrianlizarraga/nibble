...
struct Generator {
    FILE* out_fd;
    StrBucketList* text_lines;
    StrBucketList* data_lines;
...
char** emit_line(StrBucketList* sbl, const char* format, ...)
{
    char** line_ptr = NULL;
    ...
    if (format)
    {
        AllocatorState mem_state = allocator_get_state(generator.tmp_mem);
        {
            ...
            line_ptr = sbl_add(sbl, tmp_line, size);
        }
        allocator_restore_state(mem_state);
    }
    else
    {
        line_ptr = sbl_add(sbl, NULL, 0);
    }
...
bool gen_gasm(Allocator* gen_mem, Allocator* tmp_mem, Scope* scope, const char* output_file)
{
    ...
    generator.text_lines = new_sbl(gen_mem, bucket_cap);
    generator.data_lines = new_sbl(gen_mem, bucket_cap);
    ...
    ftprint_out("Num data lines: %lu, Num text lines: %lu\n", generator.data_lines->num_strs,
                generator.text_lines->num_strs);
