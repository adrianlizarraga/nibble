...
char* IR_print_instr(Allocator* arena, IR_Instr* instr)
{
    ...
    switch (instr->kind)
    {
        case IR_INSTR_ADD:
        {
            ftprint_char_array(&dstr, false, "add <%s> %s, %s",
                               type_name(instr->_add.type),
                               IR_print_op_rm(arena, &instr->_add.dst),
                               IR_print_op_rmi(arena, &instr->_add.src));
            break;
        }
        case IR_INSTR_SUB:
        {
            ftprint_char_array(&dstr, false, "sub <%s> %s, %s",
                               type_name(instr->_sub.type),
                               IR_print_op_rm(arena, &instr->_sub.dst),
                               IR_print_op_rmi(arena, &instr->_sub.src));
            break;
        }
        case IR_INSTR_SHR:
        {
            ftprint_char_array(&dstr, false, "shr <%s> %s, %s",
                               type_name(instr->_shr.type),
                               IR_print_op_rm(arena, &instr->_shr.dst),
                               IR_print_op_rmi(arena, &instr->_shr.src));
            break;
        }
        case IR_INSTR_NEG:
        {
            ftprint_char_array(&dstr, false, "neg <%s> %s", type_name(instr->_neg.type), IR_print_op_rm(arena, &instr->_neg.dst));
            break;
        }
        case IR_INSTR_MOV:
        {
            ftprint_char_array(&dstr, false, "mov <%s> %s, %s",
                               type_name(instr->_mov.type),
                               IR_print_reg(arena, instr->_mov.dst),
                               IR_print_op_ri(arena, &instr->_mov.src));
            break;
        }
        case IR_INSTR_STORE:
        {
            ftprint_char_array(&dstr, false, "store <%s> [%s], %s",
                               ...
                               IR_print_mem(arena, &instr->_store.dst),
                               IR_print_op_ri(arena, &instr->_store.src));
            break;
        }
        case IR_INSTR_LOAD:
        {
            ftprint_char_array(&dstr, false, "load <%s> %s, [%s]",
                               type_name(instr->_load.type),
                               IR_print_reg(arena, instr->_load.dst),
                               IR_print_mem(arena, &instr->_load.src));
            break;
        }
        case IR_INSTR_LADDR:
        {
            ftprint_char_array(&dstr, false, "laddr %s, [%s]",
                               IR_print_reg(arena, instr->_laddr.dst),
                               IR_print_mem(arena, &instr->_laddr.mem));
            break;
        }
        case IR_INSTR_RET:
        {
            ftprint_char_array(&dstr, false, "ret <%s> %s", type_name(instr->_ret.type), IR_print_reg(arena, instr->_ret.src));
            break;
        }
        case IR_INSTR_TRUNC:
        {
            ftprint_char_array(&dstr, false, "trunc <%s> %s, <%s> %s",
                               type_name(instr->_trunc.dst_type),
                               IR_print_reg(arena, instr->_trunc.dst),
        ...
        case IR_INSTR_ZEXT:
        {
            ftprint_char_array(&dstr, false, "zext <%s> %s, <%s> %s",
                               type_name(instr->_zext.dst_type),
                               IR_print_reg(arena, instr->_zext.dst),
        ...
        case IR_INSTR_SEXT:
        {
            ftprint_char_array(&dstr, false, "sext <%s> %s, <%s> %s",
                               type_name(instr->_sext.dst_type),
                               IR_print_reg(arena, instr->_sext.dst),
        ...
        case IR_INSTR_CMP:
        {
            ftprint_char_array(&dstr, false, "cmp <%s> %s, %s",
                               type_name(instr->_cmp.type),
                               IR_print_op_rm(arena, &instr->_cmp.op1),
                               IR_print_op_rmi(arena, &instr->_cmp.op2));
            break;
        }
        case IR_INSTR_JMPCC:
        {
            ftprint_char_array(&dstr, false, "jmp_%s %u", 
                               ir_cond_names[instr->_jmpcc.cond],
                               instr->_jmpcc.jmp_target);
            break;
        }
        case IR_INSTR_SETCC:
        {
            ftprint_char_array(&dstr, false, "set_%s %s", 
                               ir_cond_names[instr->_setcc.cond],
